...
(** Output indentation.
	@param n	Number of indentation. *)
let rec output_indent out n =
	if n <> 0 then begin
		output_string out indent;
		output_indent out (n-1)
	end
...
and output_command out cmd indent =
	output_indent out indent;
	match cmd with
	| NOP ->
		fprintf out "NOP\n"
	| SEQ(c1, c2)	->
		fprintf out "SEQ(\n";
		output_command out c1 (indent + 1);
		fprintf out ",\n";
		output_command out c2 (indent + 1);
		output_char out '\n';
		output_indent out indent;
		fprintf out ")"
	| REPEAT (n, cmd) ->
		...
		output_expr out n;
		output_string out ",\n[\n";
		output_command out cmd (indent + 1);
		output_char out '\n';
		output_indent out indent;
		fprintf out "])"
	| MAKE (n, e) ->
	...
	| PRINT e ->
		...
		output_string out ")"
	| PRINTS s ->
		fprintf out "PRINTS(\"%s\")" s
	| IF(cond, cmd1, cmd2) ->
		output_string out "IF(";
		output_cond out cond;
		output_string out ",[\n";
		output_command out cmd1 (indent + 1);
		output_char out '\n';
		output_indent out indent;
		output_string out "], [\n";
		output_command out cmd2 (indent + 1);
		output_indent out indent;
		output_string out "])"
	| CALL_CMD (name, _, args) ->
		...
		List.iter
			(fun arg ->
				output_string out ",\n";
				output_indent out (indent + 1);
				output_expr out arg) args;
		fprintf out "\n";
		output_indent out indent;
		fprintf out ")"
	| SYSCALL (cmd, args) ->
